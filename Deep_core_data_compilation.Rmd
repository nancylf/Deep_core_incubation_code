...
##### 13. Determine total net C respiration as C-CO2, C-CH4, total C respired, and C-CO2e. Additionally, calculate the percentage of total C respired at various time points during the course of the incubations.
  ...
  * Percent of total C mineralized of the original amount of soil TOC will be calculated at days 30, 150, 365
    
  * Percent of C-CO2e released that was contributed by C-CO2 vs C-CH4 will be calculated at days 30, 150, 365

  * C-CO2:C-CH4 and C-CO2:(C-CH4 as C-CO2e) will be calculated at days 30, 150, 365
  
  * Determine how much C-CO2, C-CH4, and total C (C-CO2 + C-CH4) was produced across the sediment column in one year (e.g. gC-CO2/m2/yr), and on an averaged, per day basis (e.g. gC-CO2/m2/d)
...
tot_resp_ana_co2 <- inc_df2 %>%
  ...
  select(core_ID:timepoint, TOC_perc_wt, total_resp_ugC_CO2 = total_gas_resp_ugC, total_resp_ugC_CO2_g = total_gas_resp_ugC_g, total_resp_ugC_CO2_gC = total_gas_resp_ugC_gC)

# ----- ANA CH4 -----
tot_resp_ana_ch4 <- inc_df2 %>%
  filter(headspace_treatment == "anaerobic", gas_sampled == "CH4", timepoint %in% c(8:10, 24:26, 35:37)) %>%
  select(core_ID:timepoint, TOC_perc_wt, total_resp_ugC_CH4 = total_gas_resp_ugC, total_resp_ugC_CH4_g = total_gas_resp_ugC_g, total_resp_ugC_CH4_gC = total_gas_resp_ugC_gC) %>%
...
# add average bulk density column from sample_key_data sheet
avg_bulk_density <- read_excel(sample_key_data, "Bulk density data")
tot_resp_co2e <- left_join(tot_resp_co2e, avg_bulk_density) 
...
avg_tot_resp_co2e <- tot_resp_co2e %>%
  ...
  group_by(headspace_treatment, depth_ID, replicate = cut(replicate, breaks = seq(0, 9, by = 3), labels = c("1-3", "4-6", "7-9")), timepoint = cut(timepoint, breaks = seq(0, 45, by = 15))) %>%
            ...
            #TOC and bulk density - only to keep these columns in the new df
  summarise(TOC_perc_wt = mean(TOC_perc_wt),
            avg_bulk_density_g_cm3 = mean(average_bulk_density_g_cm3),
            
            #C-CO2/g sed and C-CO2/gC
            avg_total_resp_ugC_CO2_g = mean(total_resp_ugC_CO2_g), 
...
# calculate % of total C mineralized of the original soil TOC at days 30, 150, 365
...
# note that these calculations use the averages across the three timepoints at the beginning of the incubations (~30 days), the middle (~150 days), and the end (~365 days)
avg_tot_resp_co2e <- avg_tot_resp_co2e %>%
  mutate(perc_total_C_resp = NA,
         perc_of_TOC_mineralized = ((avg_total_C_resp_ugC_g / 10^(6)) / (TOC_perc_wt / 100)) * 100,
...
# Determine how much C-CO2, C-CH4, and total C (C-CO2 + C-CH4) was produced across the sediment column in one year, and on an average, per-day basis. Use a copy of the df avg_tot_resp_co2e_day365 to do these calculations
avg_tot_resp_co2e_day365_copy <- avg_tot_resp_co2e_day365 %>%
  mutate(gC_CO2_cm3 = (avg_total_resp_ugC_CO2_g / (10^6)) * avg_bulk_density_g_cm3,
         gC_CH4_cm3 = (avg_total_resp_ugC_CH4_g / (10^6)) * avg_bulk_density_g_cm3,
         total_C_gC_cm3 = (avg_total_C_resp_ugC_g / (10^6)) * avg_bulk_density_g_cm3,
         depth_ID = as.numeric(depth_ID))

# sediment column production at 4C and day ~365, averaged across shallow, intermediate, and deep depths
sed_column_production_4C <- avg_tot_resp_co2e_day365_copy %>%
  filter(temp_treatment_C == 4) %>%
  select(1:5, 34:36) %>%
  group_by(headspace_treatment, depth_ID = cut(depth_ID, breaks = c(0, 5, 9, 12), labels = c("1-5", "6-9", "10-12")), temp_treatment_C) %>%
  summarize(avg_gC_CO2_cm3 = mean(gC_CO2_cm3),
            avg_gC_CH4_cm3 = mean(gC_CH4_cm3),
            avg_total_C_gC_cm3 = mean(total_C_gC_cm3))

# sediment column production at 10C and day ~365, averaged across shallow, intermediate, and deep depths
sed_column_production_10C <- avg_tot_resp_co2e_day365_copy %>%
  filter(temp_treatment_C == 10) %>%
  select(1:5, 34:36) %>%
  group_by(headspace_treatment, depth_ID = cut(depth_ID, breaks = c(0, 5, 9, 12), labels = c("1-5", "6-9", "10-12")), temp_treatment_C) %>%
  summarize(avg_gC_CO2_cm3 = mean(gC_CO2_cm3),
            avg_gC_CH4_cm3 = mean(gC_CH4_cm3),
...
# sediment column production at 20C and day ~365, averaged across shallow, intermediate, and deep depths
sed_column_production_20C <- avg_tot_resp_co2e_day365_copy %>%
  filter(temp_treatment_C == 20) %>%
  select(1:5, 34:36) %>%
  group_by(headspace_treatment, depth_ID = cut(depth_ID, breaks = c(0, 5, 9, 12), labels = c("1-5", "6-9", "10-12")), temp_treatment_C) %>%
  summarize(avg_gC_CO2_cm3 = mean(gC_CO2_cm3),
            avg_gC_CH4_cm3 = mean(gC_CH4_cm3),
...
# calculate sediment column production as g/m2/day and g/m2/yr using depth increments for shallow (0m - 5.99m = 599cm), intermediate (5.99m - 15.99m = 1000cm), deep (15.99m - 19.99m = 400cm)
sed_column_production_day365 <- full_join(sed_column_production_4C, sed_column_production_10C) %>%
  full_join(sed_column_production_20C)

sed_column_production_day365 <- sed_column_production_day365 %>%
  group_by(temp_treatment_C, headspace_treatment, depth_ID) %>% 
  mutate(avg_gC_CO2_m2_yr = ifelse(depth_ID == "1-5", avg_gC_CO2_cm3 * 599 * 10^(4), ifelse(depth_ID == "6-9", avg_gC_CO2_cm3 * 1000 * 10^(4), avg_gC_CO2_cm3 * 400 * 10^(4))),
         avg_gC_CO2_m2_d = avg_gC_CO2_m2_yr / 365,
         avg_gC_CH4_m2_yr = ifelse(depth_ID == "1-5", avg_gC_CH4_cm3 * 599 * 10^(4), ifelse(depth_ID == "6-9", avg_gC_CH4_cm3 * 1000 * 10^(4), avg_gC_CH4_cm3 * 400 * 10^(4))),
         avg_gC_CH4_m2_d = avg_gC_CH4_m2_yr / 365,
         avg_total_C_gC_m2_yr = ifelse(depth_ID == "1-5", avg_total_C_gC_cm3 * 599 * 10^(4), ifelse(depth_ID == "6-9", avg_total_C_gC_cm3 * 1000 * 10^(4), avg_total_C_gC_cm3 * 400 * 10^(4))),
         avg_total_C_gC_m2_d = avg_total_C_gC_m2_yr / 365)
```
...
# table_folder_path <- "/Users/nancylf/Documents/UC Berkeley/Research/NGEE Arctic/NSF Goldstream Project/Incubations/Data Compilation/R Output_Summary Tables/2024"
# 
...
# 
# # table for sediment column production (C-CO2, C-CH4, total C), averaged across three time points at the end of incubations
# save_csv(sed_column_production_day365, table_folder_path, "2024-07-22", "Avg_Sed_Col_Production_3_Timepoints_Day365")
# 
